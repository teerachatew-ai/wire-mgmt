...
router.get('/performance', (req, res) => {
  ...
  for (let i = 5; i >= 0; i--) {
    ...
    const revenue = monthRevenueOf(m);
...
function buildPayrollDetail(month: string) {
  ...
  const members = rawMembers.map((m: any) => {
    ...
    // ยอดค่าแรง/จำนวนที่ตัด แยกตามชนิดสายไฟ (เฉพาะรอบจ่ายนี้) — ใช้แสดงเป็นคอลัมน์แยกในตารางสรุปรวม
    const productAgg: Record<string, { name: string; color: string; wage: number; qty: number }> = {};
    for (const r of rows) {
      if (!productAgg[r.product_name]) productAgg[r.product_name] = { name: r.product_name, color: r.color, wage: 0, qty: 0 };
...
router.get('/wage-reconcile', (req, res) => {
  ...
  const rows = prepare(`
    SELECT p.id, p.name, p.color, p.unit, p.wage_per_unit as wage,
      -- คืนงานดี ตามรอบจ่าย (payroll / pay_cycle)
  ...
  const products = rows.map((p: any) => {
    ...
    return {
      ...
      fg_open, fg_close, dFG,
      wage_billed, wage_dFG, wage_timing, wage_extra, wage_payroll,
      reserve_open, reserve_close,
    };
  });
  ...
  res.json({
    ...
    products,
    totals: {
      wage_billed: sum('wage_billed'),        // A — ค่าแรงตามยอดวางบิล/ส่งออก
