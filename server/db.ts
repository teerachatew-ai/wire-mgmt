...
`);
...

  {
    ...
    // จำนวนที่บันทึกไว้ตอนแจกงานครั้งแรก (ยอดตามป้ายมัด) — เก็บไว้เทียบกับ quantity ปัจจุบัน
    // ผลต่าง = ของในมัดขาด/เกินจากที่โรงงานแจ้ง (สมาชิกนับแล้วมาแจ้ง เจ้าหน้าที่แก้ยอดเบิก)
    // เอาไปหักยอด "รับจริง" ของล็อตนั้นให้อัตโนมัติ ไม่ต้องกรอกซ้ำอีกที่
    if (!issueCols.includes('orig_quantity')) {
      db.exec(`ALTER TABLE issues ADD COLUMN orig_quantity REAL`);
      // ใบเบิกเก่าถือว่ายอดที่มีอยู่คือยอดตั้งต้น (ไม่ย้อนไปสร้างส่วนต่างจากการแก้ในอดีต)
      db.exec(`UPDATE issues SET orig_quantity = quantity WHERE orig_quantity IS NULL`);
    }
  }
