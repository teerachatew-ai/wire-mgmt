...
export default function DaySummary({ groups, note, unitLabel = 'รวม', memberCount, title = '📊 สรุปยอดต่อประเภท' }: { groups: SumGroup[]; note?: string; unitLabel?: string; memberCount?: number; title?: string }) {
  if (!groups.length) return null;
  const total = groups.reduce((s, g) => s + (g.qty || 0), 0);
  return (
    <div className="card">
      <div className="flex items-center justify-between mb-2 flex-wrap gap-1">
        <span className="text-sm font-semibold text-gray-700">{title}{note ? ` — ${note}` : ''}</span>
